...
// 80 bytes header parts with example values
...
void clearNonceFileIfTooLarge() {
  File file = LittleFS.open(NONCE_FILE, "r");
  if (file && file.size() > 1000000) { // If file over 100kb clear
    file.close();
...
void printNonceFileContents() {
  File file = LittleFS.open(NONCE_FILE, "r");
  if (!file) {
    Serial.println("❌ Failed to open nonce file for reading");
    return;
  }

  Serial.println("📄 Nonce file contents:");
  while (file.available()) {
    String line = file.readStringUntil('\n');
    line.trim();
    Serial.println(line);
...
void vHandleSerialCommands(){
      if (Serial.read() == 'v') {
      Serial.println("ESP8266_SHA256d_PoolMiner_v7");
